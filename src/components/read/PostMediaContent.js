...
class PostMediaContent extends Component {
  ...
  render () {
    ...
    if (this.props.carousel) {
      fullMediaArr = this.props.mediaArr
    } else {
      fullMediaArr = this.props.medium
    }
    ...
    return (
      <div style={{display: 'inline-block', width: '70vw', height: 'calc(100vh - 52px)', boxSizing: 'border-box'}}>
        ...
        {this.props.carousel &&
          <Carousel initialSlideHeight={1028} initialSlideWidth={1200}>
            {this.props.mediaArr.map((medium, i) => {
              return (
                <img key={i} src={medium.imageUrl} style={{width: '100%', height: '100%', objectFit: 'cover'}} />
              )
            })}
          </Carousel>
        }
      ...
      //     </div>
      //   }
      //   <div style={{position: 'fixed', top: '52px', left: '35vw', width: '65vw', height: 'calc(100vh - 52px)', display: 'inline-block', verticalAlign: 'top', backgroundColor: '#F5F5F5', border: '1px solid blue'}}>
