...
class EditorMediaContent extends Component {
  render () {
    let mediaArr, imageNo = 0, videoNo = 0
    ...
    return (
      <div style={{position: 'fixed', zIndex: 0, top: '56px', left: '15vw', width: '45vw', height: 'calc(100vh - 56px)', display: 'inline-block', verticalAlign: 'top', backgroundColor: '#F5F5F5'}}>
        <div style={{position: 'relative', width: '100%', padding: '16px 0'}}>
          <div style={{maxHeight: 'calc((100vh - 60px - 32px) / 7 * 6)', overflowY: 'scroll', width: 'calc(100% + 17px)', paddingRight: '17px', overscrollBehaviorY: 'contain'}}>
            <div style={{width: '45vw'}}>
              {mediaArr.map((medium, i) => {
                ...
                return <EditorMediaContentRow medium={medium} key={i} mediumNum={mediumNum} />
              })}
            </div>
          </div>
          <div style={{height: 'calc((100vh - 60px - 32px) / 7)', width: '100%', padding: '8px 24px'}}>
            <div style={{border: '1px solid rgba(60, 58, 68, 0.2)', height: '100%'}}>
              <span style={{textAlign: 'center', display: 'block', width: '100%', position: 'relative', top: '50%', transform: 'translateY(-50%)'}}>Add more image(s) and videos</span>
            </div>
          </div>
        </div>
      </div>
